...
---@param layout table|nil @Layout element to invalidate after registration. Defaults to the settings page generalSettingsLayout.
function UIHelper.registerFocusControls(controls, layout)
		...
		for _, control in ipairs(controls) do
			if not control.focusId or not FocusManager.currentFocusData.idToElementMapping[control.focusId] then
				if not FocusManager:loadElementFromCustomValues(control, nil, nil, false, false) then
					Logging.devWarning("Failed loading focus element for %s. Keyboard/controller menu navigation might be bugged.", control.id or control.name)
				end
			end
		end
		local settingsPage = g_gui.screenControllers[InGameMenu].pageSettings
		local targetLayout = layout or settingsPage.generalSettingsLayout
